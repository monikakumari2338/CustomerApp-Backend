...
public class WishlistController {
	...
	@GetMapping("/")
	public ResponseEntity<Wishlist> findUserWishlistHandler(@RequestHeader("Authorization") String jwt)
	...
	@PutMapping("/add")
	public ResponseEntity<ApiResponse> addItemToWishlist(@RequestBody AddItemRequest req,
			@RequestHeader("Authorization") String jwt) throws UserException, ProductException, CartItemException {
		User user = userService.findUserProfileByJwt(jwt);

		wishlistService.addWishlistItem(user, req);
