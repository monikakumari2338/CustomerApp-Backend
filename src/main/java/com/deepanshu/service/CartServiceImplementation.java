...
public class CartServiceImplementation implements CartService {
    ...
    public CartServiceImplementation(CartRepository cartRepository, CartItemService cartItemService, ProductService productService, UserService userService, PromotionService promotionService, UserRepository userRepository, ProductRepository productRepository, BestDealService bestDealService, PromotionRepository promotionRepository) {
    ...
    public Cart findUserCart(Long userId) {
        ...
        for (CartItem cartsItem : cart.getCartItems()) {
            ...
            System.out.println("\ncartsItem.getProduct().getPrice():"+cartsItem.getProduct().getPrice()+"\ncartsItem.getQuantity():"+cartsItem.getQuantity());
    ...
    public String addCartItem(Long userId, AddItemRequest req) throws ProductException, UserException {
        ...
        if (isPresent == null) {
            ...
            product.setPromotionalDiscountedPrice(null);
            ...
            if (product.isEligibleForBogo()) {
                ...
                // Apply the best item-wise and transaction-wise promotions only if the product is not eligible for BOGO
                try {
...
//            }
    ...
    private BigDecimal simulateApplyItemWiseDiscountToCart(Cart cart) {
        ...
        for (CartItem cartItem : cart.getCartItems()) {
            ...
            BigDecimal discountAmount = calculateDiscount(price, BigDecimal.valueOf(20.00), DiscountType.PERCENTAGE); // Example percentage discount
    ...
    private BigDecimal simulateApplyAmountOffItemWiseDiscountToCart(Cart cart) {
        ...
        for (CartItem cartItem : cart.getCartItems()) {
            BigDecimal price = BigDecimal.valueOf(cartItem.getDiscountedPrice());
            BigDecimal discountAmount = calculateDiscount(price, BigDecimal.valueOf(200.00), DiscountType.FLAT); // Example flat discount
            BigDecimal discountedPrice = price.subtract(discountAmount);
                ...
                if(totalAmount>=promotion.getMinOrderValue()){
                    ...
                    if(isForCheckBestPromotion){
                        ...
                        for(CartItem cartItem: cart.getCartItems()) {
                            ...
                            //find perItem discount
                            double perItemDiscount = productPrice * (parseFloat(promotion.getDiscountValue()) / 100);
                            double productQuantity = cartItem.getQuantity();
                            double totalProductPrice = (productPrice * productQuantity) - (splitDiscount * productQuantity);
                            cartItem.setDiscountedPrice((int) totalProductPrice);
                        ...
                        if (promocodeList.containsKey(promoCode)) {
                            // If it exists, update the existing discount value
                            double currentDiscount = promocodeList.get(promoCode);
...
// Set the updated promocodeList back to the cart
                    ...
                    if (promocodeList.containsKey(promoCode)) {
                        ...
                        cart.setPromotion_discount(cart.getPromotion_discount().subtract(BigDecimal.valueOf(discountValue)));
        ...
        if(cart!=null){
            ...
            if(cartLength>0) {
                ...
                if(totalAmount>=promotion.getMinOrderValue() && totalAmount-promotion.getMinOrderValue()>=0){
                    ...
                    //splitDiscount on each product
                    double splitDiscount=totalDiscountOnCart/countProductInCart;
                    System.out.println("\nTotal5OFFDiscount:"+totalDiscountOnCart+"OFFcountUniqueProduct:"+countProductInCart+"\nOFFSplitDiscountOnEachProduct:"+splitDiscount);
                    ...
                    if(isForCheckBestPromotion){
                        ...
                        for(CartItem cartItem: cart.getCartItems()) {
                            ...
                            double totalProductPrice = (productPrice * productQuantity) - (splitDiscount * productQuantity);
                            cartItem.setDiscountedPrice((int) totalProductPrice);
                            System.out.println("\nOFFproductPrice:" + productPrice + " " + "OFFproductQuantity:" + productQuantity + " \nOFFtotalProductPrice:" + totalProductPrice + "\n");
...
//                        // Remove the promo code from the list
    ...
    public double calculateCartTotalAmountWithPromotion(Cart cart,Set<Long>eligibleProductList,List<String>promocodeList,String promoCode){
    ...
    public double applyItemWiseTwoHundredOFFOnProduct(Long userId,boolean isForCheckBestPromotion){
        ...
        if(cart!=null) {
            ...
            Set<Long> eligibleProductList = promotionServiceImplementation.getEligibleProductIdForPromotion(promotionRepository.findByPromotionCode(promoCode).get());

            //get current Promotion
            Promotion promotion = promotionRepository.findByPromotionCode(promoCode).orElseThrow(() -> new RuntimeException("Promotion not exist in the DB:" + promoCode));
            ...
            for (CartItem cartItem : cart.getCartItems()) {
                ...
                if (eligibleProductList.contains(productId)) {
                    if(!cartItem.getProduct().isEligibleForBogo()) {
                        checkCartAmountWithoutPromotion += (cartItem.getProduct().getDiscountedPrice() * cartItem.getQuantity());
                    }
                }
            }
            ...
            if(countEligibleProductCount>0){
                ...
                for(CartItem cartItem:cart.getCartItems()){
                    ...
                    if(eligibleProductList.contains(productId)) {
                        ...
                        double productQuantity = (cartItem.getQuantity());
                        totalDiscount += ((productPrice * productQuantity) - ((productPrice * productQuantity) - splitWisePromotionOnEachProduct));
                        ...
                        if (!isForCheckBestPromotion) {
                            ...
                            cartItem.setDiscountedPrice((int) ((productPrice * productQuantity) - splitWisePromotionOnEachProduct));
//                            product.setPromotionalDiscountedPrice(String.valueOf(Math.ceil(cartItem.getProduct().getDiscountedPrice()-splitWisePromotionOnEachProduct)));
                            product.setPromotionalDiscountedPrice(String.valueOf((productPrice * productQuantity - splitWisePromotionOnEachProduct) / productQuantity));
                            productRepository.save(product);
                        }
                    }
                    }
...
//                }
                ...
                if(countEligibelItems>0) {
                    ...
                    if (totalDiscount <= promotion.getMaxDiscountOnCart()) {
                        ...
                        cart.setTotalDiscountedPrice((int) (Math.ceil(cart.getTotalDiscountedPrice() - promotion.getMaxDiscountOnCart())));
                    }
...
//                            product.setPromotionalDiscountedPrice(String.valueOf(Math.ceil(cartItem.getProduct().getDiscountedPrice()-itemDiscount)));
                            product.setPromotionalDiscountedPrice(String.valueOf((productPrice * productQuantity - itemDiscount) / productQuantity));
                            productRepository.save(product);
                        }
...
//                }
    ...
    public double anniversaryPromotionOnCart(Long userId,boolean isForCheckBestPromotion){
        ...
        User user=userRepository.findById(userId).orElseThrow(()->new RuntimeException("user not found with given userId:"+userId));
...
//                        // Remove the promo code from the list
        ...
        if(!cart.getCartItems().isEmpty()){
            ...
            cartRepository.save(cart);
             ...
             if(promocodeList.containsKey(promoCode)){
                 ...
                 cart.setPromotion_discount(cart.getPromotion_discount().subtract(BigDecimal.valueOf(promocodeList.get(promoCode))));
    ...
    public void applyBestPromotionOnCart(Long userId){
        ...
        System.out.println("applyItemWiseTwentyPercentOFFOnProduct:"+applyItemWiseTwentyPercentOFFOnProduct(userId,isForCheckBestPromotion));
        System.out.println("applyItemWiseTwoHundredOFFOnProduct:"+applyItemWiseTwoHundredOFFOnProduct(userId,isForCheckBestPromotion));
        if(applyItemWiseTwentyPercentOFFOnProduct(userId,isForCheckBestPromotion)<applyItemWiseTwoHundredOFFOnProduct(userId,isForCheckBestPromotion)){
            applyItemWiseTwentyPercentOFFOnProduct(userId,false);
        }
        else{
            applyItemWiseTwoHundredOFFOnProduct(userId,false);
        ...
        System.out.println("applyFivePercentOFFTransactionWise"+applyFivePercentOFFTransactionWise(userId,isForCheckBestPromotion));
        System.out.println("applyFiveHundredOFFTransactionWise"+applyFiveHundredOFFTransactionWise(userId,isForCheckBestPromotion));
        ...
        if (fivePercentOff > 0  &&  (fivePercentOff <= fiveHundredOff||fiveHundredOff==-1.0) && (fivePercentOff<=anniversaryOff||anniversaryOff==-1.0) && (fivePercentOff<=birthdayOff||birthdayOff==-1.0)) {
            System.out.println("\n\nfivePercentOffApplied");
            applyFivePercentOFFTransactionWise(userId, false);

        } else if (fiveHundredOff > 0  && (fiveHundredOff <= fivePercentOff||fivePercentOff==-1.0) && (fiveHundredOff<=anniversaryOff||anniversaryOff==-1.0) && (fiveHundredOff<=birthdayOff||birthdayOff==-1.0)) {
            System.out.println("\n\nfiveHundredOffApplied");
            applyFiveHundredOFFTransactionWise(userId, false);
        }else if(anniversaryOff>0 && (anniversaryOff<=fivePercentOff||fivePercentOff==-1.0) && (anniversaryOff<=fiveHundredOff||fiveHundredOff==-1.0) && (anniversaryOff<=birthdayOff||birthdayOff==-1.0)){
            anniversaryPromotionOnCart(userId,false);
        }
        else if(birthdayOff>0 && (birthdayOff<=fivePercentOff||fivePercentOff==-1.0) && (birthdayOff<=fiveHundredOff||fiveHundredOff==-1.0) && (birthdayOff<=anniversaryOff||anniversaryOff==-1.0)){
